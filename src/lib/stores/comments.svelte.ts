...
function createCommentsState() {
	...
	function loadComments(client: MatrixClient, roomId: string) {
		...
		try {
			...
			comments = events.filter((e) => e.getType() === EventType.RoomMessage).map(parseMessageEvent);
		} catch (e) {
	 ...
	 */
	async function sendFileMessage(client: MatrixClient, roomId: string, file: File, mxcUrl: string) {
