...
export function createTaskCache() {
	...
	function getTasks(): Task[] {
		return allTaskIds.map((roomId) => byRoomId.get(roomId)).filter((task): task is Task => !!task);
	}
	...
	function fetchFromRooms(client: MatrixClient, projectRoomId?: string): Task[] {
		...
		const filteredRooms = projectRoomId
			? measureSync(
					'tasks.filter_project_rooms',
					() => taskRooms.filter((room) => roomMatchesProject(room, projectRoomId)),
					{
	...
	function getByProject(projectRoomId: string): Task[] {
		...
		if (!ids) return [];
		return [...ids].map((roomId) => byRoomId.get(roomId)).filter((task): task is Task => !!task);
	}
