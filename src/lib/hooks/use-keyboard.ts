	...
	return (e: KeyboardEvent) => {
		...
		if (e.key === 'n' || e.key === 'N') {
			if (!e.ctrlKey && !e.metaKey && !e.altKey) {
				...
				window.dispatchEvent(
					new CustomEvent('tamarix:shortcut', { detail: { action: 'new_task' } })
				);
				return;
			}
		}
		...
		if (e.key >= '1' && e.key <= '5' && !e.ctrlKey && !e.metaKey && !e.altKey) {
			...
			if (statusIndex < TASK_STATUS_ORDER.length) {
				...
				window.dispatchEvent(
					new CustomEvent('tamarix:shortcut', {
						detail: { action: 'set_status', status: targetStatus }
					})
				);
			}
			return;
		...
		if (e.key === '/' && !e.ctrlKey && !e.metaKey && !e.altKey) {
			e.preventDefault();
			window.dispatchEvent(
				new CustomEvent('tamarix:shortcut', { detail: { action: 'focus_search' } })
			);
			return;
		}
		...
		if ((e.key === 't' || e.key === 'T') && !e.ctrlKey && !e.metaKey && !e.altKey) {
			e.preventDefault();
			window.dispatchEvent(
				new CustomEvent('tamarix:shortcut', { detail: { action: 'toggle_tag' } })
			);
			return;
		}
