...
export function chooseUiaFlow(session: UiaSession): UIAFlow | null {
	const supportedFlows = session.flows
		.filter((flow) =>
			flow.stages.every((stage) => session.completed.includes(stage) || SUPPORTED_STAGES.has(stage))
		)
...
export function getStageLabel(stage: string): string {
	...
	if (
		stage === AuthType.RegistrationToken ||
		stage === 'org.matrix.msc3231.login.registration_token'
	) {
...
export function createAuthDict(
	type: string,
	session?: string,
