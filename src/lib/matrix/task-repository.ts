...
import type { Task, TaskStatus, Priority, TaskType, EstimateUnit, WorklogEntry } from './types';
...
function roomToTask(room: Room): Task {
	...
	const estimateEvent = getStateEvent<{ points: number; unit: string }>(
		room,
	...
	const archiveEvent = getStateEvent<{
		archived: boolean;
		archived_by: string;
		archived_at: string;
...
function patchToWrites(
	...
	if ('status' in patch && patch.status !== undefined) {
		writes.push(
			sendStateEvent(client, roomId, TAMARIX_EVENT_TYPES.TASK_STATUS, { status: patch.status })
		);
	}
	if ('priority' in patch && patch.priority !== undefined) {
		writes.push(
			sendStateEvent(client, roomId, TAMARIX_EVENT_TYPES.PRIORITY, { level: patch.priority })
		);
	}
	if ('type' in patch && patch.type !== undefined) {
		writes.push(
			sendStateEvent(client, roomId, TAMARIX_EVENT_TYPES.TASK_TYPE, { type: patch.type })
		);
	}
	if ('dueDate' in patch && patch.dueDate !== undefined) {
		writes.push(
			sendStateEvent(client, roomId, TAMARIX_EVENT_TYPES.DUE_DATE, { date: patch.dueDate })
		);
	}
	...
	if ('assignee' in patch) {
		if (patch.assignee) {
			writes.push(
				sendStateEvent(client, roomId, TAMARIX_EVENT_TYPES.ASSIGNEE, { user_id: patch.assignee })
			);
		} else {
	...
	if ('sortOrder' in patch && patch.sortOrder !== undefined) {
		writes.push(
			sendStateEvent(client, roomId, TAMARIX_EVENT_TYPES.SORT_ORDER, { order: patch.sortOrder })
		);
	}
	if ('archived' in patch && patch.archived !== undefined) {
		const userId = client.getUserId() ?? '';
		writes.push(
			sendStateEvent(client, roomId, TAMARIX_EVENT_TYPES.TASK_ARCHIVED, {
	...
	if ('description' in patch || 'formattedDescription' in patch) {
		writes.push(
			sendStateEvent(client, roomId, TAMARIX_EVENT_TYPES.DESCRIPTION, {
		...
		initial_state: [
			{
				...
				content: { canonical: true, via: [domain] }
			},
			...(options.encrypted
				? [
						{
				...
				: []),
			{
				type: TAMARIX_EVENT_TYPES.TASK_STATUS as any,
				state_key: '',
			...
			...(options.priority
				? [
						{
							type: TAMARIX_EVENT_TYPES.PRIORITY as any,
							state_key: '',
			...
			...(options.type
				? [
						{
							type: TAMARIX_EVENT_TYPES.TASK_TYPE as any,
							state_key: '',
			...
			...(options.assignee
				? [
						{
							type: TAMARIX_EVENT_TYPES.ASSIGNEE as any,
							state_key: '',
			...
			...(options.tags?.length
				? [
						{
							type: TAMARIX_EVENT_TYPES.TAGS as any,
							state_key: '',
