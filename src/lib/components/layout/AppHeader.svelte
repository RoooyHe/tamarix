...
<header class="flex h-12 items-center gap-3 border-b border-border px-4">
	...
	>
		...
		<kbd
			class="pointer-events-none ml-1 hidden sm:inline-flex h-5 items-center gap-1 rounded border border-border bg-muted px-1.5 font-mono text-[10px] text-muted-foreground"
			>⌘K</kbd
		>
	</Button>
	...
	{#if asStatus.asUrl}
		<div
			...
			title={asStatus.asAvailable ? t('as.status_online') : t('as.status_offline')}
		>
			<span class="h-2 w-2 rounded-full {asStatus.asAvailable ? 'bg-green-500' : 'bg-red-500'}"
			></span>
		</div>
	{/if}
	...
	<DropdownMenu>
		<DropdownMenuTrigger>
			<Button variant="ghost" size="icon" class="h-8 w-8">
				...
				<Moon
					class="absolute h-4 w-4 rotate-90 scale-0 transition-transform dark:rotate-0 dark:scale-100"
				/>
			</Button>
		</DropdownMenuTrigger>
