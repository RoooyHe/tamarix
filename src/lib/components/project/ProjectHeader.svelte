<script lang="ts">
	...
	let { project, projectId, isMobile, onExportCSV, onExportJSON, onImport, onCreateTask }: Props =
		$props();
</script>

<div class="flex items-center justify-between {isMobile ? 'flex-col gap-2' : ''}">
	<div>
		<h1 class="{isMobile ? 'text-lg' : 'text-2xl'} font-bold text-foreground">
			{project?.name ?? t('breadcrumb.projects')}
		</h1>
