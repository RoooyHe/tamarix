...
<div
	class="flex flex-col rounded-lg border border-border bg-muted/30 {mobile
		? 'w-full'
		: 'w-72 shrink-0'}"
...
>
	...
	{#if !mobile || !isCollapsed}
		<div
			class="flex-1 space-y-2 p-2 overflow-y-auto min-h-[120px] transition-colors {isDragOver
				? 'bg-accent/50'
				: ''}"
		>
			...
			{#if tasks.length === 0 && !isDragOver}
				<div
					class="flex h-24 items-center justify-center rounded-md border border-dashed border-border"
				>
			...
			{#if isDragOver && tasks.length === 0}
				<div
					class="flex h-24 items-center justify-center rounded-md border-2 border-dashed border-primary bg-primary/5"
				>
					<span class="text-xs text-primary font-medium"
						>{t('kanban.drop_to', { label: getStatusLabel(status) })}</span
					>
				</div>
			{/if}
		</div>
