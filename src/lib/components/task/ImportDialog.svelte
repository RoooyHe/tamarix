<script lang="ts">
  ...
  let { open = $bindable(false), onOpenChange, client, projectRoomId, onImportComplete, createTask }: Props = $props();
...
<Dialog bind:open>
  <DialogContent class="sm:max-w-[560px]">
    ...
    {:else if step === "preview"}
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
          {#each (Object.keys(fieldMapping) as Array<keyof NormalizedImportRow>) as field (field)}
            <label class="space-y-1 text-xs">
              ...
              <select class="h-8 w-full rounded-md border border-input bg-background px-2 text-xs" bind:value={fieldMapping[field]}>
                <option value="">--</option>
          ...
          <table class="w-full text-xs">
            ...
            <tbody>
              ...
              {#if parsedRows.length > 10}
                <tr class="border-t border-border">
                  <td colspan="4" class="px-2 py-1 text-muted-foreground">...and {parsedRows.length - 10} more</td>
                </tr>
              {/if}
            </tbody>
          </table>
      ...
      <DialogFooter>
        <Button variant="outline" onclick={() => { step = "upload"; file = null; }}>{t("common.cancel")}</Button>
