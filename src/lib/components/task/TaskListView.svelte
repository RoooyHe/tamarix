...
<div class="space-y-3">
	...
	<div class="space-y-2 md:hidden">
		{#each paginatedTasks as task (task.roomId)}
			<div
				class="rounded-lg border border-border bg-card p-3 cursor-pointer {task.archived
					? 'opacity-60'
					: ''} {selectedTaskIds.has(task.roomId) ? 'ring-2 ring-primary' : ''}"
			...
			>
				<div class="flex items-center justify-between gap-2">
					<div class="flex items-center gap-2 min-w-0 flex-1">
						<button
							type="button"
							class="flex min-w-[44px] min-h-[44px] items-center justify-center"
							onclick={(e) => {
								e.stopPropagation();
							...
							<div class="flex flex-wrap items-center gap-1.5 mt-1">
								...
								{#if task.assignee}
									<span class="text-[10px] text-muted-foreground"
										>{formatSender(task.assignee)}</span
									>
								{/if}
								{#if task.archived}
		...
		<Table>
					...
					<TableHead class="px-2 py-2 w-10">
						<Checkbox
							checked={paginatedTasks.length > 0 &&
								paginatedTasks.every((t) => selectedTaskIds.has(t.roomId))}
			...
			<TableBody>
				{#each paginatedTasks as task, index (task.roomId)}
					<TableRow
						class="cursor-pointer {task.archived ? 'opacity-60' : ''} {selectedTaskIds.has(
							task.roomId
						)
							? 'bg-primary/5'
							: ''}"
						...
						{#each [...customFieldDefs.keys()].filter( (fieldName) => visibleCustomFields.has(fieldName) ) as fieldName (fieldName)}
							<TableCell class="px-3 py-2">
								<span class="text-xs text-muted-foreground"
									>{getCustomFieldDisplayValue(task.roomId, fieldName)}</span
								>
							</TableCell>
						{/each}
						<TableCell class="px-3 py-2">
