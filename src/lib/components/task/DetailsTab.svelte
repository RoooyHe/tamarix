<script lang="ts">
	...
	function parseCommitNotice(
		content: string
	): { provider: string; repo: string; branch: string; hash: string; message: string } | null {
...
<div class="space-y-3">
		...
		{#if task.estimate}
			<div>
				...
				<div class="text-sm">
					...
					{task.estimate!.unit === 'story_points'
						? t('task.estimate.story_points')
						: task.estimate!.unit === 'hours'
							? t('task.estimate.hours')
							: t('task.estimate.days')}
				</div>
			</div>
		{/if}
	</div>
	...
	<div class="flex items-center gap-2">
		...
		<VersionSelect {versions} value={currentVersion} onValueChange={handleVersionChange} />
	</div>
	...
	<div>
		...
		{/if}
		<div class="flex gap-2">
			<Input class="h-7 text-xs" placeholder={t('external_link.label')} bind:value={newLinkLabel} />
			<Input
				...
				{#each commitLinks as commit (commit.eventId)}
					<div class="flex items-center gap-2 rounded-md border border-border px-2 py-1.5 text-xs">
						...
						<span class="ml-auto shrink-0 text-muted-foreground"
							>{commit.provider} · {commit.branch}</span
						>
					</div>
				{/each}
			</div>
