<script lang="ts">
	...
	async function addRelation() {
		...
		await sendStateEvent(
			client,
			roomId,
			TAMARIX_EVENT_TYPES.RELATION,
			{
				rel_type: newRelType,
				target_room: targetTask.roomId
			},
			targetTask.roomId
		); // use target room as state_key for uniqueness
					...
					{#if entry.targetTask}
						...
						>
							{#if entry.targetTask.ticketId}
								<span class="font-mono text-xs text-muted-foreground mr-1"
									>{entry.targetTask.ticketId}</span
								>
							{/if}
