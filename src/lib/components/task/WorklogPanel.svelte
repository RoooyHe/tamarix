...
<div class="space-y-4">
		...
		<div class="space-y-2">
			...
			{#if estimateVsActual.actual > estimateVsActual.estimated}
				<p class="text-xs text-destructive">
					{Math.round(estimateVsActual.actual - estimateVsActual.estimated)}h {t(
						'worklog.estimate_vs_actual'
					).toLowerCase()}
				</p>
			{/if}
		</div>
	...
	<div class="space-y-3">
		...
		<div class="flex gap-2">
			...
			<div class="flex-1">
				<Input placeholder={t('worklog.note_placeholder')} bind:value={newNote} />
			</div>
			<Button
				size="icon"
