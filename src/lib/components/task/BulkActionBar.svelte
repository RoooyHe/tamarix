<script lang="ts">
  ...
  let { selectedCount, onClear, onBulkStatus, onBulkPriority, onBulkArchive, onBulkTag }: Props = $props();
...
{#if selectedCount > 0}
  <div class="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 rounded-lg border border-border bg-card px-4 py-2 shadow-lg {isMobile.current ? 'max-w-[calc(100vw-2rem)] overflow-x-auto' : ''}">
    ...
    <DropdownMenu>
      <DropdownMenuTrigger>
        <Button variant="outline" size="sm" class="h-8 text-xs shrink-0">{t("bulk.priority")}</Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent>
    ...
    <div class="flex items-center gap-1 shrink-0">
      ...
      />
      <Button variant="outline" size="sm" class="h-8 w-8 text-xs p-0" onclick={handleAddTag} disabled={!newTag.trim()}>
        <Tag class="h-3 w-3" />
      </Button>
    </div>

    <Button variant="outline" size="sm" class="h-8 text-xs text-destructive hover:text-destructive shrink-0" onclick={onBulkArchive}>
