	...
	function navigateToTask(taskRoomId: string, projectRoomId?: string) {
		...
		if (projectRoomId) {
			...
			if (task?.projectRoomId) {
				goto(
					`/project/${encodeURIComponent(task.projectRoomId)}/task/${encodeURIComponent(taskRoomId)}`
				);
			}
		}
	}
	...
	<CommandList>
		...
		<CommandGroup heading={t('search.projects')}>
			{#each projects.projects as project (project.roomId)}
				<CommandItem value={project.name} onSelect={() => navigateToProject(project.roomId)}>
