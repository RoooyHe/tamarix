<script lang="ts">
  ...
  function handleExportStatusDistPNG() {
    if (statusDistRef) exportChartToPNG(statusDistRef, `${project?.name ?? "project"}-status-distribution.png`);
  }
  ...
  function handleExportAssigneePNG() {
    if (assigneeRef) exportChartToPNG(assigneeRef, `${project?.name ?? "project"}-assignee-workload.png`);
  }

  function handleExportVersionPNG() {
    if (versionRef) exportChartToPNG(versionRef, `${project?.name ?? "project"}-version-progress.png`);
  }
...
<div class="space-y-6">
  ...
  <div class="flex items-center gap-3 flex-wrap">
    ...
    <div class="ml-auto flex items-center gap-2">
      <Select
        ...
        onValueChange={(v) => { timeRange = Number(v) as TimeRange; }}
      >
  ...
  {:else}
    <div class="grid gap-6 lg:grid-cols-2">
      ...
      <!-- Trend Chart -->
      <div bind:this={trendRef} class="rounded-lg border border-border bg-card p-5 space-y-4 lg:col-span-2">
        <div class="flex items-center gap-2">
          <TrendingUp class="h-5 w-5 text-muted-foreground" />
