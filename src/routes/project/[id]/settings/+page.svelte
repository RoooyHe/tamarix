<script lang="ts">
  ...
  async function handleCreateTemplate() {
    ...
    await createTaskTemplate(auth.client, projectId, {
      ...
      defaultTags: newTemplateTags.trim() ? newTemplateTags.split(",").map(s => s.trim()).filter(Boolean) : undefined
    });
    newTemplateName = "";
    newTemplateTitle = "";
    newTemplateDesc = "";
...
<div class="space-y-6">
  ...
  {#if project}
    ...
    <div class="rounded-lg border border-border bg-card p-6 space-y-4">
      ...
      <form onsubmit={(e) => { e.preventDefault(); handleCreateTemplate(); }} class="space-y-3">
        <div class="grid grid-cols-2 gap-2">
          <div class="space-y-1">
            <label class="text-xs font-medium">{t("template.name")}</label>
            <Input bind:value={newTemplateName} placeholder="Bug Report" class="h-8 text-sm" required />
          </div>
          <div class="space-y-1">
            <label class="text-xs font-medium">{t("task.title")}</label>
            <Input bind:value={newTemplateTitle} placeholder={t("template.default_values")} class="h-8 text-sm" />
          </div>
        </div>
...
<Select type="single" bind:value={newTemplateType}>
    ...
    <div class="rounded-lg border border-border bg-card p-6 space-y-4">
      ...
      <form onsubmit={(e) => { e.preventDefault(); handleCreateField(); }} class="space-y-3">
        <div class="grid grid-cols-3 gap-2">
          <div class="space-y-1">
            <label class="text-xs font-medium">{t("custom_field.label")}</label>
            <Input bind:value={newFieldLabel} placeholder="Story Points" class="h-8 text-sm" required />
          </div>
          <div class="space-y-1">
    ...
    <div class="rounded-lg border border-border bg-card p-6 space-y-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div class="space-y-1">
          ...
          <Input id="git-repo-url" bind:value={gitRepoUrl} placeholder="https://github.com/org/repo" class="h-8 text-sm" />
        </div>
      </div>
      ...
      <div class="flex items-end gap-2">
        <div class="space-y-1 flex-1">
          ...
          <Input id="git-webhook-secret" bind:value={gitWebhookSecret} class="h-8 font-mono text-xs" />
        </div>
        <Button type="button" variant="outline" size="sm" onclick={handleGenerateSecret}>
          <RefreshCw class="mr-1 h-3.5 w-3.5" />
          {t("git.generate_secret")}
        </Button>
        <Button size="sm" onclick={handleSaveGitConfig} disabled={!gitRepoUrl.trim() || !gitWebhookSecret.trim()}>
          <Save class="mr-1 h-3.5 w-3.5" />
          {t("git.save_config")}
        </Button>
      </div>
