<script lang="ts">
	...
	let releaseNotesText = $derived.by(() => {
		...
		const versionTasks = tasks.tasks.filter((task) => {
			...
			return (
				(versionEvent.getContent() as { version_key: string }).version_key === releaseNotesVersion
			);
		});
	...
	let versionProgress = $derived.by(() => {
		...
		for (const version of versions) {
			...
			const versionTasks = tasks.tasks.filter((task) => {
				const room = auth.client?.getRoom(task.roomId);
				if (!room) return false;
				const versionEvent = room.currentState.getStateEvents(
					'com.tamarix.task_version' as any,
					''
				);
				if (!versionEvent) return false;
				...
				<div class="rounded-lg border border-border bg-card p-5 space-y-3">
					<div class="flex items-center justify-between">
						<div class="flex items-center gap-3">
							...
							<Badge
								variant={version.status === 'released'
									? 'default'
						...
						<div class="flex items-center gap-2">
							{#if version.status === 'planned'}
								<Button
									size="sm"
									variant="outline"
									onclick={() => handleStatusChange(version, 'released')}
								>
					...
					{#if version.releaseDate}
						<p class="text-xs text-muted-foreground">
							{t('version.release_date')}: {version.releaseDate}
						</p>
					{/if}
...
<Dialog bind:open={releaseNotesOpen}>
	<DialogContent class="sm:max-w-[520px]">
		...
		<pre
			class="whitespace-pre-wrap text-sm bg-muted p-4 rounded-md max-h-80 overflow-auto">{releaseNotesText}</pre>
		<DialogFooter>
