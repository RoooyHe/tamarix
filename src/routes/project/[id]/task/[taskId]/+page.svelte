<script lang="ts">
	...
	async function handleUploadComplete(
		results: Array<{ mxcUrl: string; fileName: string; mimeType: string; size: number }>
	) {
		if (!auth.client || !task) return;
...
{#if task}
	<div class="space-y-4 pb-20 md:pb-4">
		...
		{#if isMobile.current}
			<button
				type="button"
				class="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground min-h-[44px]"
				onclick={goBack}
			>
		...
		<Collapsible bind:open={metadataOpen} class={isMobile.current ? 'border rounded-lg p-2' : ''}>
			{#if isMobile.current}
				<CollapsibleTrigger
					class="flex items-center gap-1 text-sm font-medium text-foreground w-full mb-2 min-h-[44px]"
				>
					{#if metadataOpen}
		...
		<Tabs bind:value={activeTab}>
			<TabsList class="w-full overflow-x-auto hidden md:flex">
				<TabsTrigger value="comments"
					>{t('task.comments')} ({commentsStore.comments.length})</TabsTrigger
				>
				<TabsTrigger value="attachments"
					>{t('task.attachments')} ({allAttachments.length})</TabsTrigger
				>
			...
			<TabsContent value="worklog" class="mt-4">
				{#if auth.client}
					<WorklogPanel
						client={auth.client}
						roomId={task.roomId}
						estimateHours={task.estimate?.unit === 'hours' ? task.estimate.points : undefined}
					/>
				{/if}
			</TabsContent>
			...
			{#if isMobile.current}
				<div
					class="fixed bottom-0 left-0 right-0 z-50 border-t bg-background flex justify-around safe-area-pb"
				>
					<button
						type="button"
						class="flex flex-col items-center justify-center py-2 px-3 min-h-[44px] min-w-[44px] {activeTab ===
						'comments'
							? 'text-primary'
