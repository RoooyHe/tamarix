<script lang="ts">
	...
	onMount(() => {
		...
		const handleShortcut = (event: Event) => {
			...
			if (detail.action === 'new_task') {
				...
				if (firstSelected) {
					goto(
						`/project/${encodeURIComponent(projectId)}/task/${encodeURIComponent(firstSelected)}`
					);
				}
			} else if (
				detail.action === 'set_status' &&
				detail.status &&
				selection.selectedTaskIds.size > 0
			) {
	...
	async function handleTaskReorder(taskId: string, status: TaskStatus, newIndex: number) {
		...
		const nextOrder = computeSortAtPosition(
			columnTasks,
	...
	async function handleImportTask(data: {
		name: string;
		status?: TaskStatus;
		priority?: Priority;
		type?: TaskType;
	...
	async function handleBulkPriority(priority: Priority) {
		if (!auth.client) return;
		await tasks.bulkUpdatePriority(
			auth.client,
	...
	async function handleShortcutTag() {
		...
		await Promise.all(
			[...selection.selectedTaskIds].map((id) => {
	...
	async function handleShortcutDueDate() {
		...
		await Promise.all(
			[...selection.selectedTaskIds].map((id) =>
				tasks.updateTaskDueDate(auth.client!, id, dueDate.trim())
			)
		);
	}
</script>
...
<div class="space-y-4 pb-20 md:pb-4">
	...
	{:else if currentView === 'board'}
		<KanbanBoard
			...
			onTaskClick={(t) =>
				goto(`/project/${encodeURIComponent(projectId)}/task/${encodeURIComponent(t.roomId)}`)}
