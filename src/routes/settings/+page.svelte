	...
	<div class="space-y-4">
		<div class="flex items-center justify-between gap-3">
			...
			<Button
				variant="outline"
				size="sm"
				onclick={() => auth.client && account.loadThreePids(auth.client)}
			>
			...
			<div class="space-y-2">
				...
				<div class="flex gap-2">
					...
					<Button
						variant="outline"
						size="sm"
						onclick={requestEmailBind}
						disabled={!emailInput || account.isSaving}
					>
						<Link class="h-4 w-4" />
			...
			<div class="flex items-center justify-between">
				<div>
					...
					<p class="text-xs text-muted-foreground">
						Local search uses client-side filtering. AS search uses the Application Service backend
		...
		<div class="rounded-lg border border-border bg-card p-4 space-y-4">
			...
			{#if asStatus.asUrl}
				<div class="flex items-center justify-between">
					...
					<div class="flex items-center gap-2">
						<span
							class="h-2 w-2 rounded-full {asStatus.asAvailable ? 'bg-green-500' : 'bg-red-500'}"
						></span>
	...
	<div class="space-y-4">
		<div class="flex items-center justify-between gap-3">
			...
			<Button
				variant="outline"
				size="sm"
				onclick={() => integrations.loadConnections(asStatus.asUrl)}
					...
					{#if connection}
						<div class="space-y-1">
							...
							{#if connection.lastSyncAt}
								<p class="text-xs text-muted-foreground">
									{t('integrations.last_sync', { time: connection.lastSyncAt })}
								</p>
							{/if}
						</div>
					{:else}
