<script lang="ts">
	...
	async function handleContinueUia() {
		...
		if (
			...
			await submitRegistration(
				createAuthDict(currentStep.type, uiaSession.session, { token: registrationToken.trim() })
			);
			return;
		}
...
<div class="flex min-h-screen items-center justify-center bg-background p-4">
	<div class="w-full max-w-md space-y-6">
		<div class="flex flex-col items-center gap-2">
			<div
				class="flex h-12 w-12 items-center justify-center rounded-xl bg-primary text-primary-foreground font-bold text-xl"
			>
				T
			</div>
				...
				<div class="flex items-center justify-between gap-3">
					<div>
						<h2 class="text-sm font-semibold text-foreground">
							{t('register.verification_required')}
						</h2>
