<script lang="ts">
  ...
  onMount(() => {
    ...
    viaServers = params.getAll("via").flatMap(value => value.split(",")).filter(Boolean);
    ...
    {:else if !auth.isLoggedIn}
      <div class="rounded-lg border border-border bg-card p-4 space-y-4 text-left">
        ...
        <div class="flex gap-2">
          ...
          <Button class="flex-1" variant="outline" href={registerUrl}>{t("register.create_account")}</Button>
        </div>
      </div>
    {:else if isJoining}
