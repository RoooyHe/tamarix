<script lang="ts">
	...
	let effectiveSource = $derived(ui.searchSource === 'as' && asStatus.asAvailable ? 'as' : 'local');
	...
	let localResults = $derived(
		searchTasks(tasks.tasks, debouncedSearchQuery, auth.client ?? undefined)
	);
...
<div class="space-y-4">
	...
	<div class="relative">
		...
		<Input class="pl-10 pr-10" placeholder={t('search.placeholder')} bind:value={searchQuery} />
		{#if searchQuery}
			<button
	...
	<div class="space-y-2">
		...
		<div class="flex flex-wrap items-center gap-1.5">
			...
			{#each TASK_STATUS_ORDER as s}
				<button
					class="rounded-full border px-2.5 py-0.5 text-xs transition-colors {filterStatus.has(s)
		...
		<div class="flex flex-wrap items-center gap-1.5">
			...
			{#each PRIORITY_ORDER as p}
				<button
					class="rounded-full border px-2.5 py-0.5 text-xs transition-colors {filterPriority.has(p)
						? 'bg-primary text-primary-foreground border-primary'
						: 'border-border text-muted-foreground hover:bg-accent'}"
		...
		<div class="flex flex-wrap items-center gap-1.5">
			...
			{#each taskTypes as tp}
				<button
					class="rounded-full border px-2.5 py-0.5 text-xs transition-colors {filterType.has(tp)
						? 'bg-primary text-primary-foreground border-primary'
						: 'border-border text-muted-foreground hover:bg-accent'}"
		...
		{#if hasActiveFilters}
			<div class="flex items-center gap-2 pt-1">
				...
				<span class="text-xs text-muted-foreground">
					{Math.min(filteredResults.length, SEARCH_RESULT_LIMIT)} / {filteredResults.length}
					{t('search.tasks').toLowerCase()}
				</span>
			</div>
		{/if}
	</div>
