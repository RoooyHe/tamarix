<script lang="ts">
  ...
  async function handleQuickCreate(data: { name: string; topic?: string; status: any; priority: any; type: any; assignee?: string }) {
...
<div class="space-y-6 pb-20 md:pb-6">
        ...
        <div class="space-y-1.5">
          {#each recentEntries as entry (entry.taskId)}
            <button
              ...
              onclick={() => goto(`/project/${encodeURIComponent(entry.projectRoomId)}/task/${encodeURIComponent(entry.taskId)}`)}
            >
              <History class="h-3.5 w-3.5 text-muted-foreground shrink-0" />
              <span class="text-sm truncate flex-1">{entry.taskTitle}</span>
              <span class="text-[10px] text-muted-foreground shrink-0">{new Date(entry.visitedAt).toLocaleDateString()}</span>
            </button>
          {/each}
        </div>
      {/if}
  ...
  <!-- Overdue Tasks -->
  <OverdueTaskList tasks={myTasks} onTaskClick={(t) => goto(`/project/${encodeURIComponent(t.projectRoomId ?? '')}/task/${encodeURIComponent(t.roomId)}`)} />
