...
interface ReportsStore {
  ...
  versionProgress: { version: string; todo: number; in_progress: number; review: number; done: number; closed: number }[];
  ...
  getVersionProgress(tasks: Task[], versions: VersionInfo[]): { version: string; todo: number; in_progress: number; review: number; done: number; closed: number }[];
}
```
